...
def test_rolling_24h_means_window_ending_at_latest_hour():
    ...
    recs += [_hourly("Pune", "pm25", f"2026-06-20T{h:02d}:00:00Z", 40.0 + h) for h in range(13)]
    ...
    assert abs(r.value - (sum(40.0 + h for h in range(13, 24)) + sum(40.0 + h for h in range(13))) / 24) < 1e-9


def test_rolling_24h_excludes_pollutant_below_min_hours():
    recs = [_hourly("Pune", "no2", f"2026-06-20T{h:02d}:00:00Z", 20.0) for h in range(6)]  # 6 < 12
...
def test_rolling_24h_ignores_rows_older_than_window():
    ...
    recs = [_hourly("Pune", "pm25", f"2026-06-20T{h:02d}:00:00Z", 50.0) for h in range(12)]
...
def test_rolling_24h_per_city_latest_hour():
    ...
    a = [_hourly("A", "pm25", f"2026-06-20T{h:02d}:00:00Z", 30.0) for h in range(12)]
    b = [_hourly("B", "pm25", f"2026-06-19T{h:02d}:00:00Z", 60.0) for h in range(12)]
...
def test_rolling_24h_skips_none_values():
    recs = [_hourly("Pune", "pm25", f"2026-06-20T{h:02d}:00:00Z", None) for h in range(14)]
    assert agg.rolling_24h(recs, min_hours=12) == []
