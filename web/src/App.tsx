...
export default function App() {
  ...
  useEffect(() => {
    fetchCityList()
      .then((l) => setRefreshedAt(l.refreshed_at ?? null))
      .catch(() => {});
    fetchCities()
      .then((cs) => {
        setCities(cs);
  ...
  const headline: HeadlineVM = useMemo(() => {
    ...
    if (live) {
      if (standard === "eu") {
        base.band = live.aqi.eu.band;
        base.category = live.aqi.eu.category;
      } else {
        base.index = live.aqi[standard].index;
        base.category = live.aqi[standard].category;
      }
      ...
      for (let i = history.length - 1; i >= 0; i--) {
        ...
        if (standard === "eu") {
          if (r.eu_band) {
            base.band = r.eu_band;
            base.category = r.eu_band;
            dominant = split(r.eu_dominant);
    ...
    if (domKey) {
      ...
      const v = live
        ? live.pollutants[domKey]?.value
        : (lastRow?.[domKey as keyof DailyRow] as number | null);
  ...
  const weather: WeatherVM | null = useMemo(() => {
    if (live?.weather)
      return {
        temp_c: live.weather.temp_c ?? null,
        rh_pct: live.weather.rh_pct ?? null,
        precip_mm: live.weather.precip_mm ?? null,
        wind_ms: live.weather.wind_ms ?? null,
      };
    if (lastRow)
      return {
        temp_c: lastRow.temp_c,
        rh_pct: lastRow.rh_pct,
        precip_mm: lastRow.precip_mm,
        wind_ms: lastRow.wind_ms,
      };
    return null;
  }, [live, lastRow]);
  ...
  const cityOptions: Option[] = useMemo(
    () =>
      cities.map((c) => ({
        value: c.city,
        label: c.city,
        hint: c.naqi != null ? `${c.naqi}` : undefined,
      })),
    [cities],
  );
  ...
  return (
    <div className="min-h-full">
      ...
      <main id="main" className="mx-auto max-w-6xl px-5 py-6">
        {err && (
          <p className="mb-4 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-red-700 dark:text-red-300">
            {err}
          </p>
        )}
        ...
        {page === "methodology" ? (
          ...
          <div className="flex flex-col gap-6">
            <ErrorBoundary label="Headline">
              <Headline standard={standard} vm={headline} loading={histLoading && !live} />
            </ErrorBoundary>
            ...
            {history.length > 1 && (
              <>
                <ErrorBoundary label="Trend">
                  <TrendChart rows={history} standard={standard} dark={dark} />
                </ErrorBoundary>
                <ErrorBoundary label="Monthly pattern">
                  <MonthlyHeatmap rows={history} standard={standard} dark={dark} />
                </ErrorBoundary>
                <ErrorBoundary label="Year-by-year">
                  <YearSummary rows={history} standard={standard} dark={dark} />
                </ErrorBoundary>
                <ErrorBoundary label="Pollutant trends">
                  <PollutantTrend rows={history} dark={dark} />
                </ErrorBoundary>
                <ErrorBoundary label="Exceedance">
                  <Exceedance rows={history} standard={standard} dark={dark} />
                </ErrorBoundary>
                <ErrorBoundary label="Weather overlay">
                  <WeatherOverlay rows={history} standard={standard} dark={dark} />
                </ErrorBoundary>
                <ErrorBoundary label="Compare">
                  <Compare available={cityNames} current={city} standard={standard} dark={dark} />
                </ErrorBoundary>
              </>
            )}
          </div>
