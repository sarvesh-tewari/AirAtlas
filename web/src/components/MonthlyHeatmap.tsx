    ...
    const pieces = cfg.numeric
      ? (() => {
          ...
          return cfg.bands.map((b) => {
            const p = { min: lo, max: b.max ?? cfg.max, color: b.color };
            lo = (b.max ?? cfg.max) + 1;
            return p;
          });
        })()
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (p: { data: [number, number, number] }) =>
          `${MONTHS[p.data[0]]} ${years[p.data[1]]}: ${p.data[2]}`,
      },
      xAxis: {
        type: "category",
        data: MONTHS,
      ...
      series: [
        {
          type: "heatmap",
          data,
          itemStyle: { borderColor: dark ? "#171a1f" : "#fff", borderWidth: 2 },
    ...
    <section className="card p-5">
      <div className="mb-3">
        <SectionTitle
          ...
          color="#7c3aed"
          eyebrow="Seasonality"
          info="Average AQI per calendar month across all years, so seasonal patterns (e.g. winter peaks) stand out."
        >
          Monthly pattern
        </SectionTitle>
      </div>
      {data.length ? (
        <EChart
          option={option}
          height={Math.max(150, years.length * 42 + 60)}
          ariaLabel="Average AQI by month and year heatmap"
        />
      ) : (
        <p className="py-8 text-center text-sm text-body">No data.</p>
      )}
      <p className="mt-2 text-xs text-muted">
        Average {cfg.numeric ? "AQI" : "band"} per month, showing the seasonal pattern across years.
      </p>
    </section>
  );
}
